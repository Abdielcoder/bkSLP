...
export const storeAccesos = firestore.collection("loginAccesos");

export const addEventLogListener = async (uid, event, dataX) => {
  const timestamp = await getActualDate();
  await firestore.collection('crudEventLogs')
    .doc(event).collection(timestamp.today)
    .doc(timestamp.timestamp).set({
      user_uid: uid,
      ...dataX
    }).then(() => {
